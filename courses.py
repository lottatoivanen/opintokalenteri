...
def add_course(name, description, user_id):
    sql = """INSERT INTO courses (name, description, user_id) VALUES (?, ?, ?)"""
    db.execute(sql, [name, description, user_id])

def get_course(course_id):
    sql = """SELECT courses.id, courses.name, courses.description, courses.user_id, users.username AS username FROM courses JOIN users ON users.id = courses.user_id WHERE courses.id = ?"""
